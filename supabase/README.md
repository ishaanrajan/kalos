...
3. **Run `0011_drake_bot.sql`, then `0012_drake_bot_photo_log.sql`, then
   `0039_drake_daily_once.sql`** in the SQL editor. The first two schedule
   the function and create the no-repeat tracking table (required, the
   function's first call will error without it); the third cuts the
   schedule down to once daily, at 15:30 UTC (~9:30am Mountain) —
   `cron.schedule` upserts by job name, so running 0039 after 0011 just
   updates the same job in place.
